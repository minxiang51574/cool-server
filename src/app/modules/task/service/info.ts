...
export class TaskInfoService extends BaseService {
  ...
  async addOrUpdate(params) {
    ...
    await this.getOrmManager().transaction(async transactionalEntityManager => {
      ...
      if (params.status === 1) {
        ...
        const { opts } = await this.taskInfoQueue.add(params, {
          jobId: params.id,
          removeOnComplete: true,
          removeOnFail: true,
          repeat: jobOp,
  ...
  async updateStatus() {
    ...
    for (const job of result) {
      const task = await this.taskInfoEntity.findOne({ id: parseInt(job.id) });
      if (task) {
        setTimeout(async () => {
          // 2秒后清空任务
          ...
          if (nextTime && nextTime.getTime() <= new Date().getTime() - 999) {
            ...
            this.taskInfoQueue.removeRepeatable(JSON.parse(task.repeatConf));
          }
        }, 2000);
      }
    }
